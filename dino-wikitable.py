...
stopclean = lambda name, place: name.replace(place + " ", "", 1)

timestr = lambda secs: (str(secs//3600).zfill(2)+":"+str((secs//60)%60).zfill(2)+":"+str(secs%60).zfill(2)).rstrip("00").rstrip(":")

mins = lambda td: ("%f" % (td.total_seconds()/60)).rstrip("0").rstrip(".")

rowspan = lambda e: 1 if e == [] else sum(rowspan(x[1]) for x in e)

def r_a(a, i=0, labels = [["Linie"], ["Linienverlauf", "Haltestellen", "Strecke"], ["Fahrtzeit"], ["Einschränkung"], ["Wochentage", "Abfahrtszeiten"]]):
    for x, y in a:
        print(rowspan(y), end=" ")
        print("-"*i, end="")
        for ei, ex in enumerate(x):
            print("\""+labels[i][ei]+"\": " + str(ex), end="")
            if ei < len(x) - 1:
                print(" | ", end="")
            else:
                print("")
...
def fahrplanarray(lineids, place):
    a = []
    for lineid in lineids:
        ...
        for courseid in line.courses:
            ...
            #print(course)
            a_c = [["'''" + stopclean(course.stopfrom, place) + "'''&nbsp;→ '''" + stopclean(course.stopto, place) + "'''", len(course.stops), str(round(course.distance/1000, 1)).replace(".", ",").rstrip("0").rstrip(",") + " km"], []]
            fahrzeiten = {}
            ...
            for fahrzeit in fahrzeiten:
                ...
                for restrictiontext in restrictions:
                    ...
                    for daytype in restrictions[restrictiontext]:
                        ...
                        takttext = "-".join(takt(sorted(restrictions[restrictiontext][daytype])))
...
if __name__ == "__main__":
    ...
    """
    lineids = [50514, 50522, 50544, 50104, 50901]
    ...
    a = fahrplanarray(lineids, place)
    print(dumps(a), end="\n\n")
    r_a(a)
    print("")

    head = "Diese Tabelle wurde komplett aus DINO-Fahrplandaten vom VRR generiert und ist sehr experimentell.\n\n== Busverkehr ==\n=== Liniennetz ===\n==== … ====\n"
    ...
    for line in a:
        wikitext += "\n|-\n"
        wikitext += "| " + linepre + f" rowspan=\"{rowspan(line[1])}\" | " + line[0][0] + "\n"
        for course in line[1]:
            for cs in course[0]:
                wikitext += f"| rowspan=\"{rowspan(course[1])}\" | " + str(cs) + "\n"
            for tg in course[1]:
                wikitext += f"| rowspan=\"{rowspan(tg[1])}\" | " + tg[0][0] + "\n"
                for res in tg[1]:
                    wikitext += f"| rowspan=\"{rowspan(res[1])}\" | " + res[0][0] + "\n"
                    for dtt in res[1]:
                        for dtts in dtt[0]:
                            wikitext += f"| rowspan=\"{rowspan(dtt[1])}\" | " + str(dtts) + "\n"
                        wikitext += "|-\n"
